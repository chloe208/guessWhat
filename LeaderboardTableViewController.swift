...
class LeaderboardTableViewController: UITableViewController {
    ...
    func readValue(){
        ...
        if sqlite3_prepare(db, query, -1, &stmnt, nil) != SQLITE_OK {
        ...
        while(sqlite3_step(stmnt) == SQLITE_ROW ){
            let id = Int (sqlite3_column_int(stmnt, 0))
            let username = String(cString: sqlite3_column_text(stmnt, 1))
            let score = Int(sqlite3_column_int(stmnt, 2))
            let s = Score(id: id, username: username, score: score)
            scores.append(s)
        }
    }
    ...
    func addSample(){
        ...
        for i in 1...10{
            ...
            if sqlite3_prepare(db, insert, -1, &stmnt, nil) != SQLITE_OK{
                let err = String(cString: sqlite3_errmsg(db))
                print("error preparing statement: \(err)")
                return
            }
            if sqlite3_bind_text(stmnt, 1, "Score\(i)", -1, nil) != SQLITE_OK{
                let err = String(cString: sqlite3_errmsg(db))
                print("error biding make: \(err)")
                return
            }
            if sqlite3_bind_int(stmnt, 2, Int32(i)) != SQLITE_OK{
                let err = String(cString: sqlite3_errmsg(db))
    ...
    override func viewDidLoad() {
        ...
        if sqlite3_open(file.path, &db) != SQLITE_OK{
            ...
            let create =
            "CREATE TABLE IF NOT EXISTS Cars (id INTEGER PRIMARY KEY AUTOINCREMENT, make TEXT, price INTEGER)"
